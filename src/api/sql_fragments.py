...
OVERRIDE_AGG_SUBQUERY = """(
    SELECT stock_item_name,
           ...
           BOOL_OR(CASE WHEN field_name='current_stock' THEN hold_from_po END) AS stock_hold_from_po,
           MAX(CASE WHEN field_name='total_velocity' THEN override_value END) AS total_vel_override_value,
           BOOL_OR(CASE WHEN field_name='total_velocity' THEN is_stale END) AS total_vel_override_stale,
           BOOL_OR(CASE WHEN field_name='total_velocity' THEN hold_from_po END) AS total_vel_hold,
           MAX(CASE WHEN field_name='wholesale_velocity' THEN override_value END) AS wholesale_vel_override_value,
           BOOL_OR(CASE WHEN field_name='wholesale_velocity' THEN hold_from_po END) AS wholesale_vel_hold,
           MAX(CASE WHEN field_name='online_velocity' THEN override_value END) AS online_vel_override_value,
           BOOL_OR(CASE WHEN field_name='online_velocity' THEN hold_from_po END) AS online_vel_hold,
           MAX(CASE WHEN field_name='store_velocity' THEN override_value END) AS store_vel_override_value,
           BOOL_OR(CASE WHEN field_name='store_velocity' THEN hold_from_po END) AS store_vel_hold,
