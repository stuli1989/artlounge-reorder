...
def run_targeted_recompute(db_conn, category_names: list[str]) -> dict:
    """Recompute positions + metrics for all SKUs in the given categories.

    Used after reorder intent / supplier changes to immediately update
    affected SKU metrics without running the full pipeline.
    """
    if not category_names:
        return {"skus_recomputed": 0, "brands_recomputed": 0}
    ...
    # Delegate to party-based recompute by finding a synthetic "all SKUs" path.
    # We reuse the full recompute_skus_for_party machinery but bypass the
    ...
    for m in metrics_batch:
        ...
        xyz = m["xyz_class"]
        supplier = supplier_map.get(m["category_name"])
        supplier_buf_override = supplier["buffer_override"] if supplier else None
        buf = compute_safety_buffer(abc, xyz, buffer_settings,
                                     supplier_override=supplier_buf_override,
                                     use_xyz=use_xyz_global)
        m["safety_buffer"] = buf
